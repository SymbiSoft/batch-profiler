...
class _UI:
	...
	def new_profile(self):
		...
		self._prepare_main()
		if len(self.globalsettings.keys()) == 0:
	...
	def rename_profile(self):
		...
		for x in self.globalsettings.keys():
			if not x == self.profile:
				...
				self.newsettings[str(name)] = self.globalsettings[x]
		settings.save(self.newsettings)
		self._prepare_main()
		self.set_profile(manual = map(lambda x:x[1], self.list_for_listbox)[0])
	...
	def add_prog_search(self):
		apps.show_list_search(self.profile)

	def add_prog_search_task(self):
		apps.show_list_search(self.profile, task = 1)
	...
	def _auto_set_profile(self):
		try:
			self.set_profile(manual = self.read_profile())
		except:
			self.set_profile(manual = map(lambda x:x[1], self.list_for_listbox)[0])
...
class _Core:
	...
	def add_program(self):
		...
		if self.task:
			name = self.lista_task[self.list_box.current()][0]
		else:
			name = self.lista_applicazioni[self.list_box.current()][0]
	...
	def _update_lb_profile(self, bind = 1):
		...
		if self.programmiscelti == []:
			...
			if bind:
				self.list_box.bind(key_codes.EKeyBackspace, self.delete_program)
		appuifw.app.body = self.list_box
